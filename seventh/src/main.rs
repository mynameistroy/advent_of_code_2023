...
fn get_card_value_v2(card: char) -> usize {
    match card {
        '2' | '3' | '4' | '5' | '6' | '7' | '8' | '9' => card.to_digit(10).unwrap() as usize,
        'T' => 10,
        'J' => 1,
...
fn get_joker_type_and_count(hand_str: &str) -> (char, usize) {
    ...
    for card in &hand {
        ...
        if get_card_value(*card) > get_card_value(highest) {
            highest = *card;
        }
    }
...
fn get_hand_rank_v2(hand_str: &str) -> usize {
    let mut hand = hand_str.chars().collect::<Vec<char>>();
    let (highest, jokers) = get_joker_type_and_count(hand_str);
...
fn part_two() {
    ...
    for line in PUZZLE_INPUT.lines() {
        ...
        let(current_hand_str, current_bid) = line.split_once(' ').unwrap();
        // derive the hand rank based on the hank
        let current_hand_rank = get_hand_rank_v2(current_hand_str);
        // create Hand struct based on rank and hand
        let current_hand = Hand {
            hand: current_hand_str.to_string(),
            bid: current_bid.parse().unwrap(),
        ...
        for current_hand in rv {
            ...
            for hand_index in 0..sorted_hands.len() {
                ...
                for card_index in 0..5 {
                    let current_card = current_cards[card_index];
                    let existing_card = vec_cards[card_index];
                    let current_card_value = get_card_value_v2(current_card);
                    let existing_card_value = get_card_value_v2(existing_card);
